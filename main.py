                    ...
                    'subcategories': [
                        {
                            ...
                            'value': f'{cat["name"]} > {entry["subcategory"]}',
                        }
                    ]
...
def build(repo_root: str) -> None:
    ...
    repo_stars = (
        format_stars_short(repo_self['stars']) if 'stars' in repo_self else None
    )
    ...
    print(
        f'Built single page with {len(parsed_groups)} groups, {len(categories)} categories'
    )
